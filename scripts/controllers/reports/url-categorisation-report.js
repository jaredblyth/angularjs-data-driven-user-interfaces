...
angular.module('controller.reports.urlCategorisationReport', ['ngCsv']).controller('UrlCategorisationReportCtrl', ['$scope', '$modal', 'reportService', 'smartTableUtils', '$filter', 'shellService', '$rootScope', 'dateUtils',
    function ($scope, $modal, reportService, smartTableUtils, $filter, shellService, $rootScope, dateUtils) {

        console.log('created url categorisation report controller.');
        ...
        $scope.model = {
			...
			smartTableReportsColumns: [
				...
				{'label':'Suggested Category','map':'expectedCategory', isSortable:false},
				{'label':'Resulting Category','map':'responseCategory', isSortable:false},
        ...
        $scope.init = function () {
            $scope.model.searchCriteria.selectedRequestStatus = $scope.model.searchCriteria.requestStatus[0];
        ...
        $scope.generate = function () {
			...
			searchCriteria.request_status = $scope.model.searchCriteria.selectedRequestStatus.status;
			...
			if ($scope.model.reportType == 'email')
				
				{
					...
					reportService.generateCategorisationReport(searchCriteria).then(function () {
						shellService.saveComplete();
			...
			if ($scope.model.reportType == 'display')
				
				{
					...
					reportService.displayCategorisationReport(searchCriteria).then(function (data) {
						...
						$.each(data, function(idx, obj) {
							...
							if (obj.expectedCategory == null) {obj.expectedCategory = '-';}
							if (obj.responseCategory == null) {obj.responseCategory = '-';}
							if (obj.requestTime == null) {obj.requestTime = '-';}
							$scope.model.smartTableReportsRows.push({"url":(obj.url),"domain":(obj.domain),"centreCode":(obj.centreCode),"misId":(obj.misId),"expectedCategory":(obj.expectedCategory),"responseCategory":(obj.responseCategory),"requestTime":(obj.requestTime)});
						});
